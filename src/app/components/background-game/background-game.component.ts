...
export class BackgroundGameComponent implements OnInit {
  ...
  itens: string[] = [
    ...
    "frasco", "frascos",
    "varal",
    "cesta", "caixa", "caixas", "cestas",
    "janela", "janelas",
    "câmera", "camera", "cameras", "câmeras",
    "cadeira", "cadeiras",
    "corda", "cordas",
    "estátua", "estátuas", "estatua", "estatuas", "estatueta", "estatuetas"
  ];
...
@Input() handleEvent($event: CountdownEvent): void {
  ...
  keyEvent(event: KeyboardEvent) {
    if (event.keyCode == KEY_CODE.DOWN_ARROW){
      ...
      if ((<HTMLInputElement>document.getElementById('inputBox')).value !== "") {
        console.log(event);
        this.activateItem((<HTMLInputElement>document.getElementById('inputBox')).value.toLowerCase());
  ...
  ngOnInit(): void {
    ...
    this.openDialog(modalText).afterClosed().subscribe(result => {
